...
exports.activateUnifiedHud = activateUnifiedHud;
exports.deactivateUnifiedHud = deactivateUnifiedHud;
const vscode = __importStar(require("vscode"));
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const client_1 = require("../api/client");
const hudClient_1 = require("../api/hudClient");
const precomputed_data_1 = require("../precomputed-data");
const RANK_COLORS = {
...
// STATE
// ============================================================================
let tokenDecorationTypes = [];
let ghostDecorationType = null;
let currentWordDecorationType = null;
let saliencyDecorationTypes = [];
let timeout = null;
...
let demoMode = 'normal';
// Scripted demo state
let scriptedHighlightDecorations = [];
let scriptedGhostDecoration = null;
let scriptedSaliencyDecorations = [];
// ============================================================================
// PRECOMPUTED DATA
// ============================================================================
...
function loadPrecomputedData() {
    try {
        ...
        if (precomputed_data_1.PRECOMPUTED_DATA && precomputed_data_1.PRECOMPUTED_DATA.code) {
            precomputedData = precomputed_data_1.PRECOMPUTED_DATA;
            usePrecomputed = true;
...
function getPrecomputedGhost(line, char) {
    if (!usePrecomputed || !precomputedData?.ghosts)
        return null;
    const key = `${line}:${char}`;
    return precomputedData.ghosts[key] || null;
}
function getPrecomputedSaliency(line, char) {
    if (!usePrecomputed || !precomputedData?.saliencies)
        return null;
    const key = `${line}:${char}`;
    return precomputedData.saliencies[key] || null;
}
function getPrecomputedTokenRanks() {
    if (!usePrecomputed || !precomputedData?.token_ranks)
        return null;
...
// ACTIVATION
// ============================================================================
function activateUnifiedHud(context) {
    console.log('Unified HUD: Activating...');
    console.log('Extension path:', context.extensionPath);
    console.log('Extension URI:', context.extensionUri?.toString());
    extensionContext = context;
    loadPrecomputedData();
    context.subscriptions.push(vscode.commands.registerCommand('cerebrasHud.toggleHud', toggleHud), vscode.commands.registerCommand('cerebrasHud.setDemoMode', setDemoMode), vscode.commands.registerCommand('cerebrasHud.scriptedHighlight', scriptedHighlight), vscode.commands.registerCommand('cerebrasHud.scriptedGhost', scriptedGhost), vscode.commands.registerCommand('cerebrasHud.scriptedSaliency', scriptedSaliency), vscode.commands.registerCommand('cerebrasHud.clearScriptedDemo', clearScriptedDemo));
    updateHud();
    context.subscriptions.push(vscode.workspace.onDidChangeTextDocument(() => debouncedUpdate()), vscode.window.onDidChangeActiveTextEditor(() => updateHud()), vscode.window.onDidChangeTextEditorSelection(() => debouncedUpdate()));
    console.log('Unified HUD: Activated');
}
function toggleHud() {
    hudEnabled = !hudEnabled;
    vscode.window.showInformationMessage(`Cerebras HUD: ${hudEnabled ? 'Enabled' : 'Disabled'}`);
...
async function updateHud() {
    ...
    try {
        ...
        if (isDemoMode && usePrecomputed) {
            ...
            clearDecorations(editor);
            if (showEntropy)
                renderTokenHighlightsFromPrecomputed(editor);
            if (showSidebar)
                renderSidebarFromPrecomputed(editor);
            if (showGhost)
                renderGhostFromPrecomputed(editor, position);
            if (showSaliency)
                renderSaliencyFromPrecomputed(editor, position);
        }
            ...
            const [analysis, ghostData, saliencyData] = await Promise.all([
                (0, client_1.analyze)(code, uri),
                (0, hudClient_1.fetchGhost)(code, uri, position.line + 1, position.character),
                (0, hudClient_1.fetchSaliency)(code, uri, position.line + 1, position.character)
            ]);
            clearDecorations(editor);
            if (showEntropy)
                renderTokenHighlights(editor, analysis);
            if (showSidebar)
                renderSidebar(editor, analysis);
            if (showGhost)
                renderGhost(editor, position, ghostData);
            if (showSaliency)
                renderSaliency(editor, saliencyData);
        }
    }
    catch (err) {
...
// RENDER - LIVE MODE
// ============================================================================
function renderTokenHighlights(editor, analysis) {
    ...
    for (const line of analysis.lines) {
        ...
        for (const tok of line.tokens) {
            ...
            if (tokenPos >= 0) {
                const range = new vscode.Range(line.line_number - 1, tokenPos, line.line_number - 1, tokenPos + trimmed.length);
...
function renderSidebar(editor, analysis) {
...
function renderGhost(editor, position, ghostData) {
    ...
    if (ghostText.trim() === ghostData.primary.token.trim())
        return;
...
function renderSaliency(editor, saliencyData) {
    if (!saliencyData?.tokens.length)
        return;
    ...
    for (const token of topTokens) {
        if (token.line < 1 || token.line > editor.document.lineCount)
            continue;
        const lineText = editor.document.lineAt(token.line - 1).text;
        const tokenLength = token.tokenText.length;
        if (token.character < 0 || token.character + tokenLength > lineText.length)
            continue;
        const range = new vscode.Range(token.line - 1, token.character, token.line - 1, token.character + tokenLength);
...
// ============================================================================
function renderTokenHighlightsFromPrecomputed(editor) {
...
function renderSidebarFromPrecomputed(editor) {
    const tokenRanks = getPrecomputedTokenRanks();
    if (!tokenRanks)
        return;
    const code = editor.document.getText();
    ...
    const lineCounts = new Array(lines.length).fill(0);
    let currentLine = 0;
    let lineStartPos = 0;
    for (const tok of tokenRanks) {
        while (currentLine < lines.length && tok.position >= lineStartPos + lines[currentLine].length + 1) {
...
function renderGhostFromPrecomputed(editor, position) {
...
function renderSaliencyFromPrecomputed(editor, position) {
...
// HELPERS
// ============================================================================
function logprobToCategory(logprob) {
...
function scriptedHighlight(highlights) {
    const editor = vscode.window.activeTextEditor;
    if (!editor)
        return;
    ...
    // Group by color
    const byColor = { green: [], yellow: [], orange: [], red: [] };
    ...
    for (const [color, items] of Object.entries(byColor)) {
        if (items.length === 0)
            continue;
        const colorDef = SCRIPTED_COLORS[color];
        const decorationType = vscode.window.createTextEditorDecorationType({
            backgroundColor: colorDef.bg,
            overviewRulerColor: colorDef.bar,
            overviewRulerLane: vscode.OverviewRulerLane.Right
        });
        const decorations = items.map(h => ({
...
function scriptedSaliency(saliency) {
    const editor = vscode.window.activeTextEditor;
    if (!editor)
        return;
