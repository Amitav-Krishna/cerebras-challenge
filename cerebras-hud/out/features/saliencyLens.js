...
exports.activateSaliencyLens = activateSaliencyLens;
exports.debouncedSaliencyUpdate = debouncedSaliencyUpdate;
exports.clearSaliencyDecorations = clearSaliencyDecorations;
exports.deactivateSaliencyLens = deactivateSaliencyLens;
...
let connectorDecorationType = null;
let saliencyTimeout = null;
let isAnalyzing = false;
let saliencyEnabled = false; // Off by default (expensive)
/**
 * Initialize the saliency lens.
 */
function activateSaliencyLens(context) {
    context.subscriptions.push(vscode.commands.registerCommand('cerebrasHud.toggleSaliency', toggleSaliency), vscode.commands.registerCommand('cerebrasHud.runSaliencyAnalysis', runSaliencyNow));
}
function toggleSaliency() {
    saliencyEnabled = !saliencyEnabled;
    vscode.window.showInformationMessage(`Saliency Lens: ${saliencyEnabled ? 'Enabled (auto)' : 'Disabled'}`);
 ...
 */
function debouncedSaliencyUpdate() {
    if (!saliencyEnabled)
        return;
    if (saliencyTimeout) {
 ...
 */
function clearSaliencyDecorations(editor) {
...
async function updateSaliency() {
    ...
    await vscode.window.withProgress({
        ...
        try {
            ...
            // Fetch saliency data
            const saliencyData = await (0, hudClient_1.fetchSaliency)(document.getText(), uri, position.line + 1, position.character);
            if (!saliencyData || !saliencyData.tokens.length)
                return;
 ...
 */
function showSaliencyHighlights(editor, cursorPosition, tokens) {
    ...
    for (const token of salientTokens) {
        ...
        // Validate position
        if (char < 0 || char + tokenLength > lineText.length)
            continue;
        // Create highlight
        const range = new vscode.Range(new vscode.Position(line, char), new vscode.Position(line, char + tokenLength));
        ...
        const decorationOptions = {
            range,
            hoverMessage: new vscode.MarkdownString(`**Saliency: ${token.klDivergence.toFixed(3)}**\n\n` +
 ...
 */
function drawConnectors(editor, cursorPosition, tokenRanges) {
 ...
 */
function deactivateSaliencyLens() {
    if (saliencyTimeout) {
        clearTimeout(saliencyTimeout);
    }
}
